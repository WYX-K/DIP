...
unsigned char *Dilation(unsigned char *tempin, int height, int width, int flag) {
...
int find(unsigned char *tempin, int n, Pixel *pixel, int height, int width) {
    int length = 0;
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if (tempin[i * width + j] == n) {
                pixel[length].y = i;
                pixel[length].x = j;
                pixel[length].value = tempin[i * width + j];
                length++;
            }
        }
    }
