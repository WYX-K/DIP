...
Image *PartialOtusImage(Image *image) {
    ...
    int width = image->Width;
    int size = height * width;
    Image *outimage = CreateNewImage(image, height, width, (char *)"#PartialOtus Image");
    unsigned char *tempin = image->data;
    unsigned char *tempout = outimage->data;
...
Image *AverFilterImage(Image *image, int number1, int number2) {
    ...
    int width = image->Width;
    int height = image->Height;
    memcpy(tempout, tempin, width * height);
    for (int i = zero1 / 2; i < height - zero1 / 2; i++) {
        for (int j = zero2 / 2; j < width - zero2 / 2; j++) {
            int res = 0;
            for (int m = 0; m < number1; m++) {
                for (int n = 0; n < number2; n++) {
                    res += tempin[(i - zero1 / 2 + m) * width + (j - zero2 / 2 + n)];
                }
            }
            tempout[i * width + j] = res / (number1 * number2);
        }
    }
